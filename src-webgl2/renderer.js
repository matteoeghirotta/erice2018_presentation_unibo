...
export function Renderer(canvas, resolution, aoResolution) {
	...
	// 2 triangles filling a quad
        ...
        var count = position.length / 9; // number of indices to render: 2 primitives
	...
	// ellipsoids
        ...
        if (view.bonds || view.lattice) {
            ...
            let attribs = webgl.buildAttribs(gl, {
                aImposter: 3,
                aPosA: 3,
                aPosB: 3,
                aRadius: 1,
    ...
    function normal(view) {
        ...
        var rect = View.getRect(view);
        var projection = glm.mat4.create();
        glm.mat4.ortho(projection, rect.left, rect.right, rect.bottom, rect.top,
		       0, range);
        var viewMat = glm.mat4.create();
        glm.mat4.lookAt(viewMat, [0, 0, 0], [0, 0, -1], [0, 1, 0]);
        var model = glm.mat4.create();
